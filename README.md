...
├── README.md
├── requirements.txt
├── run_all_baselines.sh          # HF baseline on 5x5 / 6x6 / 7x7
...
Run the three pre-trained HuggingFace models on 5×5, 6×6, and 7×7 grids:
...
baseline result (6×6 IID test set, 200 samples):
...
> Results for 5×5 and 7×7 grids are generated by running `bash run_all_baselines.sh`.
> API zero-shot results (GPT-4o-mini etc.) are generated by running `bash run_api_zeroshot.sh`.
